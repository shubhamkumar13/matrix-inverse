...
let inverse (pool : T.pool) (mat : matrix) : matrix =
  let (n, p), _m = mat in
  if n = p then (
    ...
    T.parallel_for ~start:1 ~finish:n
      ~body:(fun i ->
        (* Printf.printf "Augmented Matrix\n"; *)
...
let _ =
  ...
  let size = Sys.argv.(1) |> int_of_string in
  let pool = T.setup_pool ~num_domains:num_domains in
  let matrix =
    Array.init size (fun _ -> Array.init size (fun _ -> Random.float 100.))
  in
  let _inv_matrix =
    inverse pool ((Array.length matrix, Array.length matrix.(0)), matrix)
  in
